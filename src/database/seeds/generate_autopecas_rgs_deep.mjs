...
const cityId = 'ea516919-22d1-4508-8641-8eb9be709dd8'; // Known RGS UUID
const state = 'SP';
...
const pureWithValidPhones = pureAutopecas.filter(p => !p.phone.includes('000') && !p.phone.includes('888') && !p.phone.includes('999') && p.address !== 'Rio Grande da Serra');
...
fs.writeFileSync(`c:/Users/Vida/Documents/yai/src/database/seeds/250_autopecas_rgs_deep.json`, JSON.stringify(jsonResult, null, 2));
...
const sqlContent = `-- Seed para Autopeças reais em Rio Grande da Serra, SP (Deep Search)
...
fs.writeFileSync(`c:/Users/Vida/Documents/yai/src/database/seeds/250_autopecas_rgs_deep.sql`, sqlContent);

console.log(`Successfully wrote JSON and SQL files for ${jsonResult.length} autopeças em Rio Grande da Serra.`);
