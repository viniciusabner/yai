...
async function run() {
  try {
    const data = JSON.parse(fs.readFileSync('desentupidoras_sao_caetano.json', 'utf8'))

    let sql = `-- Inserção de Desentupidoras (Saneamento) em São Caetano do Sul (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/150_real_desentupidoras_sao_caetano.sql', sql)
    console.log('SQL generated at src/database/seeds/150_real_desentupidoras_sao_caetano.sql')
