...
async function run() {
  try {
    const hvacs = JSON.parse(fs.readFileSync('hvac_ribeirao_pires.json', 'utf8'))

    let sql = `-- Inserção de Técnicos em Ar Condicionado em Ribeirão Pires (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/139_real_hvac_ribeirao_pires.sql', sql)
    console.log('SQL generated at src/database/seeds/139_real_hvac_ribeirao_pires.sql')
