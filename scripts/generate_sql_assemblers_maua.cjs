...
async function run() {
  try {
    const assemblers = JSON.parse(fs.readFileSync('furniture_assemblers_maua.json', 'utf8'))

    let sql = `-- Inserção de Montadores de Móveis em Mauá (Apenas Verificados)
...
`
    ...
    fs.writeFileSync('src/database/seeds/117_real_furniture_assemblers_maua.sql', sql)
    console.log('SQL generated at src/database/seeds/117_real_furniture_assemblers_maua.sql')
