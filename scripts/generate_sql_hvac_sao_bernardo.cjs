...
async function run() {
  try {
    const hvacs = JSON.parse(fs.readFileSync('hvac_sao_bernardo.json', 'utf8'))

    let sql = `-- Inserção de Técnicos em Ar Condicionado em São Bernardo do Campo (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/135_real_hvac_sao_bernardo.sql', sql)
    console.log('SQL generated at src/database/seeds/135_real_hvac_sao_bernardo.sql')
