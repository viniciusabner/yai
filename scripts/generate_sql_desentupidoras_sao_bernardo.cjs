...
async function run() {
  try {
    const data = JSON.parse(fs.readFileSync('desentupidoras_sao_bernardo.json', 'utf8'))

    let sql = `-- Inserção de Desentupidoras (Saneamento) em São Bernardo do Campo (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/149_real_desentupidoras_sao_bernardo.sql', sql)
    console.log('SQL generated at src/database/seeds/149_real_desentupidoras_sao_bernardo.sql')
