...
async function run() {
  try {
    const assemblers = JSON.parse(fs.readFileSync('furniture_assemblers_diadema.json', 'utf8'))

    let sql = `-- Inserção de Montadores de Móveis em Diadema (Apenas Verificados)
...
`
    ...
    fs.writeFileSync('src/database/seeds/116_real_furniture_assemblers_diadema.sql', sql)
    console.log('SQL generated at src/database/seeds/116_real_furniture_assemblers_diadema.sql')
