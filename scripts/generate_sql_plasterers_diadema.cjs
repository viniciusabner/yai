...
async function run() {
  try {
    const plasterers = JSON.parse(fs.readFileSync('plasterers_diadema.json', 'utf8'))

    let sql = `-- Inserção de Gesseiros em Diadema (Dados Reais)
-- Cidade: Diadema
...
`
    ...
    fs.writeFileSync('src/database/seeds/123_real_plasterers_diadema.sql', sql)
    console.log('SQL generated at src/database/seeds/123_real_plasterers_diadema.sql')
