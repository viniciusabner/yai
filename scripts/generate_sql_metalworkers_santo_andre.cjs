...
async function run() {
  try {
    const metalworkers = JSON.parse(fs.readFileSync('metalworkers_santo_andre.json', 'utf8'))

    let sql = `-- Inserção de Serralheiros (Estruturas Metálicas) em Santo André (Dados Reais)
-- Cidade: Santo André
...
`
    ...
    fs.writeFileSync('src/database/seeds/141_real_metalworkers_santo_andre.sql', sql)
    console.log('SQL generated at src/database/seeds/141_real_metalworkers_santo_andre.sql')
