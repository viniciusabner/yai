...
async function run() {
  try {
    const hvacs = JSON.parse(fs.readFileSync('hvac_sao_caetano.json', 'utf8'))

    let sql = `-- Inserção de Técnicos em Ar Condicionado em São Caetano do Sul (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/136_real_hvac_sao_caetano.sql', sql)
    console.log('SQL generated at src/database/seeds/136_real_hvac_sao_caetano.sql')
