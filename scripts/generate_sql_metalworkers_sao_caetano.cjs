...
async function run() {
  try {
    const metalworkers = JSON.parse(fs.readFileSync('metalworkers_sao_caetano.json', 'utf8'))

    let sql = `-- Inserção de Serralheiros (Estruturas Metálicas) em São Caetano do Sul (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/143_real_metalworkers_sao_caetano.sql', sql)
    console.log('SQL generated at src/database/seeds/143_real_metalworkers_sao_caetano.sql')
