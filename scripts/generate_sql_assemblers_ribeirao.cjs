...
async function run() {
  try {
    const assemblers = JSON.parse(fs.readFileSync('furniture_assemblers_ribeirao_pires.json', 'utf8'))

    let sql = `-- Inserção de Montadores de Móveis em Ribeirão Pires (Apenas Verificados)
...
WHERE provider_id IN (
  ...
  WHERE (city_id = (SELECT id FROM cities WHERE slug = 'ribeirao-pires') AND category = 'montador_moveis')
...
`
    ...
    fs.writeFileSync('src/database/seeds/118_real_furniture_assemblers_ribeirao_pires.sql', sql)
    console.log('SQL generated at src/database/seeds/118_real_furniture_assemblers_ribeirao_pires.sql')
