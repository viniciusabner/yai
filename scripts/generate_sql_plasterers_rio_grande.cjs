...
async function run() {
  try {
    const plasterers = JSON.parse(fs.readFileSync('plasterers_rio_grande.json', 'utf8'))

    let sql = `-- Inserção de Gesseiros em Rio Grande da Serra (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/126_real_plasterers_rio_grande.sql', sql)
    console.log('SQL generated at src/database/seeds/126_real_plasterers_rio_grande.sql')
