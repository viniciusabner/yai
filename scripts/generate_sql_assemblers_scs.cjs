...
async function run() {
  try {
    const assemblers = JSON.parse(fs.readFileSync('furniture_assemblers_scs.json', 'utf8'))

    let sql = `-- Inserção de Montadores de Móveis em São Caetano do Sul (Apenas Verificados)
...
`
    ...
    fs.writeFileSync('src/database/seeds/115_real_furniture_assemblers_scs.sql', sql)
    console.log('SQL generated at src/database/seeds/115_real_furniture_assemblers_scs.sql')
