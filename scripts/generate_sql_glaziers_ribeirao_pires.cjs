...
async function run() {
  try {
    const glaziers = JSON.parse(fs.readFileSync('glaziers_ribeirao_pires.json', 'utf8'))

    let sql = `-- Inserção de Vidraceiros em Ribeirão Pires (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/132_real_glaziers_ribeirao_pires.sql', sql)
    console.log('SQL generated at src/database/seeds/132_real_glaziers_ribeirao_pires.sql')
