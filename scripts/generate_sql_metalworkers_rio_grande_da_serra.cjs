...
async function run() {
  try {
    const metalworkers = JSON.parse(fs.readFileSync('metalworkers_rio_grande_da_serra.json', 'utf8'))

    let sql = `-- Inserção de Serralheiros (Estruturas Metálicas) em Rio Grande da Serra (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/147_real_metalworkers_rio_grande_da_serra.sql', sql)
    console.log('SQL generated at src/database/seeds/147_real_metalworkers_rio_grande_da_serra.sql')
