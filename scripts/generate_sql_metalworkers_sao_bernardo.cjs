...
async function run() {
  try {
    const metalworkers = JSON.parse(fs.readFileSync('metalworkers_sao_bernardo.json', 'utf8'))

    let sql = `-- Inserção de Serralheiros (Estruturas Metálicas) em São Bernardo do Campo (Dados Reais)
...
`
    ...
    fs.writeFileSync('src/database/seeds/142_real_metalworkers_sao_bernardo.sql', sql)
    console.log('SQL generated at src/database/seeds/142_real_metalworkers_sao_bernardo.sql')
