...
async function run() {
  try {
    const hvacs = JSON.parse(fs.readFileSync('hvac_maua.json', 'utf8'))

    let sql = `-- Inserção de Técnicos em Ar Condicionado em Mauá (Dados Reais)
-- Cidade: Mauá
...
`
    ...
    fs.writeFileSync('src/database/seeds/138_real_hvac_maua.sql', sql)
    console.log('SQL generated at src/database/seeds/138_real_hvac_maua.sql')
