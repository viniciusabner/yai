...
async function run() {
  try {
    const glaziers = JSON.parse(fs.readFileSync('glaziers_maua.json', 'utf8'))

    let sql = `-- Inserção de Vidraceiros em Mauá (Dados Reais)
-- Cidade: Mauá
...
`
    ...
    fs.writeFileSync('src/database/seeds/131_real_glaziers_maua.sql', sql)
    console.log('SQL generated at src/database/seeds/131_real_glaziers_maua.sql')
