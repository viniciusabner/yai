...
async function run() {
  try {
    const data = JSON.parse(fs.readFileSync('desentupidoras_maua.json', 'utf8'))

    let sql = `-- Inserção de Desentupidoras (Saneamento) em Mauá (Dados Reais)
-- Cidade: Mauá
...
`
    ...
    fs.writeFileSync('src/database/seeds/152_real_desentupidoras_maua.sql', sql)
    console.log('SQL generated at src/database/seeds/152_real_desentupidoras_maua.sql')
